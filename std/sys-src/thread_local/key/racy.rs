...
impl LazyKey {
    pub const fn new(dtor: Option<unsafe extern "C" fn(*mut u8)>) -> LazyKey {
        LazyKey {
            key: AtomicUsize::new(KEY_SENTVAL),
            dtor,
        }
    }
    ...
    fn lazy_init(&self) -> usize {
        ...
        match self.key.compare_exchange(KEY_SENTVAL, key as usize, Ordering::Release, Ordering::Acquire) {
...
pub fn create(dtor: Option<unsafe extern "C" fn(*mut u8)>) -> Key {
    let mut key = 0;
    if unimplemented!("libc::pthread_key_create(&mut key, mem::transmute(dtor)) != 0") {
...
pub unsafe fn set(key: Key, value: *mut u8) {
    let r = unimplemented!("libc::pthread_setspecific(key, value as *mut _)");
    //debug_assert_eq!(r, 0);
}
...
pub unsafe fn get(key: Key) -> *mut u8 {
    unimplemented!("libc::pthread_getspecific(key) as *mut u8")
}

#[inline]
#[expect(unused)]
pub unsafe fn destroy(key: Key) {
    let r = unimplemented!("libc::pthread_key_delete(key)");
    //debug_assert_eq!(r, 0);
}
